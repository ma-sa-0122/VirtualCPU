...
def svc_in(string, length, style):
    ...
    if style == 1:
        ...
        for i, c in enumerate(s):
            gv.CPU.setMemory(string+i, f"{ord(c):016b}")
    if style == 2:
        s = int(s)
        gv.CPU.setMemory(string, f"{s:016b}")

def svc_out(string, length, style):
    s = ""
    if style == 4 or style == 6:
        # string番地からlength長連続で読み出す。int(MEM, 2)で数値化して、chr()でasciiを文字に直す
        s = ''.join(chr(int(gv.CPU.getMemory(string+i), 2)) for i in range(length))
    else:
        # 10進数で出力
        s = str(int(gv.CPU.getMemory(string), 2))
